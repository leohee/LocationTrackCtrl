...
UINT8   HostCtrlTransfer( PUINT8 DataBuf, PUINT16 RetLen );  // 执行控制传输,8字节请求码在pSetupReq中,DataBuf为可选的收发缓冲区
// 如果需要接收和发送数据,那么DataBuf需指向有效缓冲区用于存放后续数据,实际成功收发的总长度返回保存在ReqLen指向的字节变量中

void CopySetupReqPkg( const UINT8 *pReqPkt );            // 复制控制传输的请求包
UINT8 CtrlGetDeviceDescr( PUINT8 DataBuf );                    // 获取设备描述符,返回在 pHOST_TX_RAM_Addr 中
UINT8 CtrlGetConfigDescr( PUINT8 DataBuf );                    // 获取配置描述符,返回在 pHOST_TX_RAM_Addr 中
